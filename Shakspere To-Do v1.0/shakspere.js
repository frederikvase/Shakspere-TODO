...
function reload()
{
  ...
  for (var i = 0; i < close.length; i++)
  {
    ...
    if (copy[index][3] + copy[index][4] >= copy[index][1])
...
function updateProgress()
{
  ...
  for (var i = 0; i < childArray.length; i++) 
  {
    ...
    var span = childArray[i].querySelector("#unique-span-id");
    if (span)
      span.innerHTML = "<span id=\"unique-span-id\" style=\"float: left; color: var(" + (progress > 4 ? "--bg2" : "--bg1") + ");\">" + daysLeft + "</span>";
    else
      childArray[i].innerHTML += "<span id=\"unique-span-id\" style=\"float: left; color: var(" + (progress > 4 ? "--bg2" : "--bg1") + ");\">" + daysLeft + "</span>";
  }
